...
console.table(estudiantes);
console.log("Dividir el arreglo en uno nuevo eliminando ciertos elementos en base a su poscicion ")
//Cuando la funcion splice recibe un solo parametro eliminara los elementos de esa posicion en adelante 
estudiantes.slice(2)
console.table(estudiantes)
//Cuando la funcion splice recibe los parametros, se eliminann todos los elementos que no estan en su rango
estudiantes.push("Saul Goodman")
estudiantes.push("Mike Ermantraud")
estudiantes.push("Skyler White")
estudiantes.push("Jimmy McGill")
estudiantes.push("Jesse Pinkman")
console.log("Se han agregado 5 nuevos estudiantes, por lo que el arreglo es: ")
console.table(estudiantes)
console.log("Ahora ya tenemos los elementos suficientes para aplicar el metodo splice con 2 paramentros que seran 3,5")
estudiantes.splice(0,2)
console.log("Resultado en:")
console.table(estudiantes)

console.log("Ahora vamos a insertar a edgar cabrera en los elemntos de la posicion 0 y 1")
estudiantes.splice(1,0,"Al Farias")
console.log("Resultado en: ")
console.table(estudiantes)
...
//Congelamos el primer arreglo volviendolo inmutable;
...
estudiantes.push("Adrian Pereza");
estudiantes.push("Jesus Domniguez");
estudiantes.push("Al Farias");
estudiantes.push("Obed Garcia");
console.table(estudiantes);
Object.freeze(estudiantes);

console.log("Filtrado de lo sprimeros 5 elementos");
...
let nuevosEstudiantesNombre = estudiantes.filter((estudiante) => estudiante.
length>15);
console.table(nuevosEstudiantesNombre);
...
//Reduccion de elementops de un arreglo, se usa cuando debemos hacer operaciones matematicas o cuantitativas de un arreghlo, como obtener totales, la idea es reducir la lista a un valor mas simplificado
    const costosListaCompras = [15,52,50,16,90,32,50,28,105,45.2,94.10]
    //Como pódemos calculra el total de de un alista de compras de un acrrito de compras
